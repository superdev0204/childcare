...
class CenterCareController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $providers = Facility::where('is_center', 1)
                            ...
                            ->whereDate('created_date', '>=', Carbon::now()->subDays(10))
        ...
        Cache::remember('latest_centers', 21600, function () use ($providers) {
            return $providers;
        });
        
        $resources = Pages::where('categories', 'like', '%1003%')
                            ->where('approved', 1)
                            ->orderBy('created_date', 'desc')
                            ->limit(3)
                            ->get();

        return view('centercare.centercare', compact('user', 'providers', 'states', 'summary', 'cities', 'resources'));
    }
    ...
    public function state(Request $request)
    {
        ...
        if ($state->nextlevel == 'DETAIL') {
            ...
            $filename = $state->state_code . "_" . $state->state_name . "_centers";
            ...
            return view('centercare.centercare_state_detail', compact('user', 'providers', 'state', 'zipcodes', 'questions', 'page_url'));
        }

        if ($state->center_count > 0) {
            $counties = Counties::where('state', $state->state_code)
                                ->where('center_count', '>=', request()->query('display') == 'all' ? 1 : 8)
                                ->orderBy('county', 'asc')
                                ->get();
        } else {
            $counties = Counties::where('state', $state->state_code)
        ...
        Cache::remember($state->state_code . '_centers', 21600, function () use ($providers) {
            return $providers;
        });
        
        $resources = Pages::where('domain', 'childcarecenter.us')
                            ->where('approved', 1)
                            ->limit(2)
                            ->offset(3)
                            ->get();
        ...
        if (request()->query('display') == 'all') {
            return view('centercare.centercare_state_with_all_counties', compact('user', 'providers', 'state', 'counties', 'cities', 'resources', 'questions', 'page_url'));
        }

        return view('centercare.centercare_state', compact('user', 'providers', 'state', 'counties', 'cities', 'resources', 'questions', 'page_url'));
    }
    ...
    public function all_cities(Request $request)
    {
        ...
        $counties = Counties::where('state', $state->state_code)
                            ->where('center_count', '>=', 21)
                            ->orderBy('center_count', 'desc')
                            ->get();
        ...
        return view('centercare.centercare_all_cities', compact('user', 'state', 'counties', 'cities', 'resources', 'questions', 'page_url'));
    }
    ...
    public function county(Request $request)
    {
        ...
        if (request()->query('display') == 'zipcode') {
            ...
            return view('centercare.centercare_countyzip', compact('user', 'state', 'county', 'cities', 'zipcodes', 'questions', 'page_url'));
        }
        ...
        if (request()->query('amp')) {
        	return view('centercare.centercare_amp_county', compact('user', 'state', 'county', 'cities', 'zipcodes', 'providers', 'referalResources', 'questions', 'page_url'));
        }
        
        return view('centercare.centercare_county', compact('user', 'state', 'county', 'providers', 'referalResources', 'cities', 'zipcodes', 'questions', 'page_url'));
    }
    ...
    {        
        ...
        if (request()->query('type') == 'preschool') {
            $providers = Facility::where('is_center', 1)
                                ->where('approved', '>=', 1)
                                ->where('state', $city->state)
                                ->where('city', $city->city)
                                ->where('is_preschool', 1)
                                ->get();
        }
        elseif (request()->query('type') == 'religious') {
            $providers = Facility::where('is_center', 1)
                                ->where('approved', '>=', 1)
                                ->where('state', $city->state)
                                ->where('city', $city->city)
                                ->where('is_religious', 1)
                                ->get();
        }
        else {
            if ($city->center_count <= 0) {
                $providers = $this->getCentersNearbyCity($city->state, $city->city, $city->latitude, $city->longitude, 30, 10);

                $filename = $city->state . "_" . $city->city . "_centers";
                $filename = str_replace([" ","-"],"_",$filename);
                $filename = str_replace(["'","."],"",$filename);
                ...
                $filename = $city->state . "_" . $city->city . "_centers" . ($page ? "_page_" . ((int) $page) . '_per_page_20' : '');
                $filename = str_replace([" ","-"],"_",$filename);
                $filename = str_replace(["'","."],"",$filename);
        ...
        if ($city->center_count >= 20) {
            $zipcodes = $zipcodes = Zipcodes::where('state', $city->state)
                                            ->where('city', $city->city)
                                            ->where('center_count', '>=', 1)
                                            ->orderBy('zipcode', 'asc')
                                            ->get();
        } else {
            $zipcodes = $this->getZipcodesNearby($city->latitude, $city->longitude, 18, ['state' => $city->state, 'minCenterCount' => 1], 20);
        }
        ...
        if (request()->route()->parameter('format') == 'amp') {
        	return view('centercare.centercare_amp_city', compact('user', 'providers', 'state', 'city', 'zipcodes', 'page', 'topProviders', 'questions', 'page_url'));
        }

        return view('centercare.centercare_city', compact('user', 'providers', 'state', 'city', 'zipcodes', 'page', 'topProviders', 'questions', 'page_url'));
    }
    ...
    {        
        ...
        if ($zipcode->center_count <= 0) {
            if (!$zipcode->lat || !$zipcode->lng) {
                $coordinates = $this->geocode('', $zipcode->city, $zipcode->state, $zipcode->zipcode);
            ...
            $providers = $this->getCentersNearbyZipcode($zipcode->state, $zipcode->zipcode, $zipcode->lat, $zipcode->lng, 20, 10);

            $filename = $zipcode->state . "_" . $zipcode->zipcode . "_centers";
            $filename = str_replace([" ","-"],"_",$filename);
            $filename = str_replace(["'","."],"",$filename);
            ...
            $filename = $zipcode->state . "_" . $zipcode->zipcode . "_centers" . ($page ? "_page_" . ((int) $page) . '_per_page_20' : '');
            $filename = str_replace([" ","-"],"_",$filename);
            $filename = str_replace(["'","."],"",$filename);
        ...
        return view('centercare.centercare_zipcode', compact('user', 'providers', 'topProviders', 'state', 'zipcode', 'page', 'questions', 'page_url'));
    }
    
    public function geocode($street, $city, $state, $zip)
    {
        $url = "http://dev.virtualearth.net/REST/v1/Locations?o=xml&key=Am6EgbEd8PhbeXjaWux-E1RbY3JSgkp2qKorbDTPqmQ8S3loNRk_OqZ5iOwt35o7";

        $address = $street . " " . $city . " " . $state . " " . $zip;

        $requestUrl = $url . "&q=" . urlencode($address);
        ...
        $status = $xml->StatusDescription;

        if (strcmp($status, "OK") == 0) {
            $location = $xml->ResourceSets->ResourceSet->Resources->Location->Point;
    ...
    public function generateFilename($foodbank_name, $foodbank_city, $foodbank_state)
    {
        $name = str_replace(["  "," ","-","(",")","/","@","+","&"],"_",$foodbank_name);
        $name = str_replace([":",".","'",",","#","\""],"",$name);

        $city = str_replace(["  "," ","-","(",")","/","@","+","&"],"_",$foodbank_city);
        $city = str_replace([":",".","'",",","#","\""],"",$city);

        $filename = strtolower($name . "_" . $city . "_" . $foodbank_state);
        $filename = str_replace(["'","&"],"",$filename);
    ...
    public function getCentersNearbyCity($stateCode, $city, $latitude, $longitude, $distance = 10, $limit = null)
    {
    ...
    public function getCentersNearbyZipcode($stateCode, $zipcode, $latitude, $longitude, $distance = 10, $limit = null)
    {
        $query = Facility::select('facility.*')
                        ->selectRaw('((ACOS(SIN(RADIANS('.$latitude.')) * SIN(RADIANS(facility.lat)) + COS(RADIANS('.$latitude.')) * COS(RADIANS(facility.lat)) * COS(RADIANS(('.$longitude.' - facility.lng)))) * 180 / PI()) * 60 * 1.1515) AS distance')
                        ->where('facility.is_center', 1)

...
class MinnesotaReviewsCsvParser extends Command
{
    protected $signature = 'custom:parse-minnesota-reviews-csv';
    protected $description = 'Parse data using a specified parser';
    protected $file = '/datafiles/minnesota/MinnesotaReviews.csv';
    ...
    public function handle()
    {
        ...
        while (($data = fgetcsv($handle, 2000, ";")) !== false) {
            ...
            if (!$facility) {
                ...
                if ($data[6] <> "") {
                    $inspection->provider_response = $data[6];
                }
                if ($data[4] <> "") {
                    $inspection->current_status = $data[7] . " - " . $data[4];
                }

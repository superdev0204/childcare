@push('title')
    <title>Childcare Centers in Washington DC | {{ $state->state_name }} Child Daycare</title>
@endpush
...
@section('content')
    <div class="container">
        ...
        <section class="left-sect">
            ...
            <p>
                <?php if ($state->center_count > 20) :?>
                There are <?php echo number_format($state->center_count); ?> daycare centers in <?php echo ucwords(strtolower($state->state_name)); ?>. Use the ZIP code dropdown on the right
                hand side to narrow the childcare list.<br />
                <?php else :?>
                Below are the list of <?php echo $state->center_count; ?> daycare centers in <?php echo ucwords(strtolower($state->state_name)); ?>.<br />
                <?php endif; ?>

                <?php if ($state->center_count <= 20 && $state->homebase_count > 0) :?>
                You may also want to checkout <?php echo $state->homebase_count; ?> other home daycare providers in <a
                    href="/<?php echo $state->statefile; ?>_homecare"><?php echo ucwords(strtolower($state->state_name)); ?> Home Daycares</a>.<br />
                <?php elseif($state->homebase_count > 0): ?>
                If you're looking for a <a href="/homecare">Home Daycare</a> in <?php echo $state->state_name; ?>, click on <a
                    href="/<?php echo $state->statefile; ?>_homecare"><?php echo ucwords(strtolower($state->state_name)); ?> Home Daycares</a>.<br />
                <?php endif; ?>
            </p>
